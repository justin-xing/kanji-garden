...
import { X, Sparkles, Eraser, Type, Save, Pencil, Image as ImageIcon, Trash2, Eye, EyeOff } from 'lucide-react';
import type { KanjiEntry } from '../data/kanji';
import { generateMnemonic, generateKanjiVisualization } from '../services/geminiService';
...
const renderStyledText = (text: string) => {
  return text.split(/(\*\*.*?\*\*)/g).map((part, i) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return <strong key={i} className="font-bold text-indigo-jp">{part.slice(2, -2)}</strong>;
    }
    return part;
  });
};

const KanjiDetailModal: React.FC<KanjiDetailModalProps> = ({ kanji, isOpen, onClose }) => {
  ...
  const drawCanvas = () => {
    ...
    if (!visualizationImage || !showImage) {
        ...
        ctx.fillText(kanji.character, rect.width / 2, (rect.height / 2) - (size * 0.05));
    }
  ...
  const saveLabel = () => {
    if (tempLabel.trim()) {
        setShapes(prev => [...prev, { path: currentPath, label: tempLabel.trim() }]);
        setCurrentPath([]);
        setTempLabel("");
        setShowLabelInput(false);
    } else {
  ...
  const handleGenerateMnemonic = async () => {
    ...
    const result = await generateMnemonic(kanji.character, kanji.meaning, labels, kanji.mnemonic);
    setMnemonic(result);
    ...
    try {
        ...
        if (base64Image) {
            ...
            try {
                ...
                console.warn("Could not save image to localStorage (likely quota exceeded)", e);
            }
            setShowImage(true);
        }
    } catch (e) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 pointer-events-none">
      ...
      <div className="bg-paper w-full max-w-md rounded-2xl shadow-xl overflow-y-auto flex flex-col pointer-events-auto max-h-[90vh] animate-fade-in relative">
        ...
        <div className="sticky top-0 z-50 px-6 py-4 border-b border-stone-200 flex justify-between items-start bg-white/95 backdrop-blur shrink-0">
            <div>
                <h2 className="text-3xl font-serif text-ink mb-1">{kanji.character}</h2>
                <div className="flex flex-col">
                    <span className="text-lg font-medium text-stone-800">{kanji.meaning}</span>
                    <span className="text-sm text-stone-500 font-mono">{kanji.hiragana} • {kanji.romaji}</span>
                </div>
            </div>
            <button 
                onClick={onClose}
        ...
        <div className="relative w-full aspect-square bg-stone-50 overflow-hidden shrink-0 border-b border-stone-100 group">
            ...
            {visualizationImage && showImage && (
                <div className="absolute inset-0 z-0 pointer-events-none animate-fade-in">
                    <img src={visualizationImage} alt="Mnemonic visualization" className="w-full h-full object-cover" />
                </div>
            )}
            ...
            {generatingImage && (
                 <div className="absolute inset-0 z-30 bg-white/60 backdrop-blur-sm flex items-center justify-center flex-col gap-3">
                     ...
                     <p className="text-sm font-bold text-wasabi animate-pulse">Painting...</p>
                 </div>
            )}
        </div>
            ...
            {!mnemonic ? (
                ...
                <div className="flex flex-col gap-3">
                    <div className="bg-sakura/30 rounded-xl p-4 border border-sakura relative group transition-colors focus-within:border-pink-400 focus-within:bg-white">
                        <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center gap-2">
                                ...
                                <h4 className="text-xs font-bold text-pink-700 uppercase tracking-wider">Mnemonic Story</h4>
                            </div>
                    ...
                    <div className="flex gap-2">
                     ...
                     {loading && (
                        <div className="text-center text-xs text-stone-400 animate-pulse">Updating mnemonic...</div>
                    )}
                </div>
            )}
